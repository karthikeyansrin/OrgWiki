...
export type KnowledgeArticle = KnowledgeArticleSummary & { markdownContent: string; generatedAtUtc: string; relatedArticles: { key: string; title: string; summary: string }[]; citations: { sourceFileName: string; sourcePath: string; evidenceSnippet: string }[] }
export type TeamSpaceSummary = { id: string; name: string; slug: string; description: string; articleCount: number }
export type ArticleTeamSpaces = { articleKey: string; teamSpaces: { id: string; name: string; slug: string; description: string }[] }
export type PublicTeamSpaceSummary = { name: string; slug: string; description: string; articleCount: number }
export type PublicTeamSpace = { name: string; slug: string; description: string; articles: { key: string; title: string; summary: string; lastUpdatedAtUtc: string; publishedAtUtc: string }[] }
export type PublicTeamSpaceArticle = { key: string; title: string; summary: string; markdownContent: string; tags: string[]; difficulty: string; estimatedReadingMinutes: number; generatedAtUtc: string; lastUpdatedAtUtc: string; publishedAtUtc: string; relatedArticles: { key: string; title: string; summary: string }[] }
...
export async function getTeamSpaces(): Promise<TeamSpaceSummary[]> { const response = await apiFetch('/api/team-spaces'); if (!response.ok) throw new Error(await readError(response)); return response.json() as Promise<TeamSpaceSummary[]> }
export async function createTeamSpace(space: Pick<TeamSpaceSummary, 'name' | 'slug' | 'description'>): Promise<TeamSpaceSummary> { const response = await apiFetch('/api/team-spaces', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(space) }); if (!response.ok) throw new Error(await readError(response)); return response.json() as Promise<TeamSpaceSummary> }
export async function deleteTeamSpace(id: string): Promise<void> { const response = await apiFetch(`/api/team-spaces/${encodeURIComponent(id)}`, { method: 'DELETE' }); if (!response.ok) throw new Error(await readError(response)) }
export async function getArticleTeamSpaces(articleKey: string): Promise<ArticleTeamSpaces> { const response = await apiFetch(`/api/team-spaces/articles/${encodeURIComponent(articleKey)}`); if (!response.ok) throw new Error(await readError(response)); return response.json() as Promise<ArticleTeamSpaces> }
export async function updateArticleTeamSpaces(articleKey: string, teamSpaceIds: string[]): Promise<ArticleTeamSpaces> { const response = await apiFetch(`/api/team-spaces/articles/${encodeURIComponent(articleKey)}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ teamSpaceIds }) }); if (!response.ok) throw new Error(await readError(response)); return response.json() as Promise<ArticleTeamSpaces> }
export async function getPublicTeamSpaces(): Promise<PublicTeamSpaceSummary[]> { const response = await apiFetch('/api/public/spaces'); if (!response.ok) throw new Error(await readError(response)); return response.json() as Promise<PublicTeamSpaceSummary[]> }
export async function getPublicTeamSpace(slug: string): Promise<PublicTeamSpace> { const response = await apiFetch(`/api/public/spaces/${encodeURIComponent(slug)}`); if (!response.ok) throw new Error(await readError(response)); return response.json() as Promise<PublicTeamSpace> }
export async function getPublicTeamSpaceArticle(slug: string, articleKey: string): Promise<PublicTeamSpaceArticle> { const response = await apiFetch(`/api/public/spaces/${encodeURIComponent(slug)}/articles/${encodeURIComponent(articleKey)}`); if (!response.ok) throw new Error(await readError(response)); return response.json() as Promise<PublicTeamSpaceArticle> }
