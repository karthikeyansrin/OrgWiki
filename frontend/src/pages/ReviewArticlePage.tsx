...
export function ReviewArticlePage() {
  ...
  const refresh = async () => {
    ...
    await client.invalidateQueries({ queryKey: ['knowledge-home'] })
  }
  ...
  const publish = useMutation({
    mutationFn: () => publishReviewArticle(articleId),
    onSuccess: async () => {
      if (query.data?.status === 'Published') {
        await refresh()
        setNotice('Article republished. Team Spaces now show the latest published version.')
      } else {
        navigate('/review')
      }
    }
  ...
  return <section className="animate-fade-in">
    ...
    {isPublished
      ? <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-900"><span>Published articles can be refined. Save your changes, then republish to refresh the publication record and Team Spaces.</span><Link to={`/knowledge/articles/${article.key}`} className="font-semibold underline underline-offset-2">View in Knowledge Base</Link></div>
      : <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><AlertTriangle size={16} className="mr-2 inline" />This article contains AI-generated content. Confirm the draft against the immutable source evidence before approval.</div>}
      ...
      <div className="space-y-5">
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 space-y-5">
            <label className="block text-sm font-semibold text-slate-700">Title<input disabled={busy} value={draft.title} onChange={event => update('title', event.target.value)} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5 outline-none transition focus:border-teal-600 focus:ring-2 focus:ring-teal-100 disabled:bg-slate-100" /></label>
            <label className="block text-sm font-semibold text-slate-700">Summary<textarea disabled={busy} value={draft.summary} onChange={event => update('summary', event.target.value)} className="mt-2 min-h-28 w-full rounded-lg border border-slate-300 px-3 py-2.5 outline-none transition focus:border-teal-600 focus:ring-2 focus:ring-teal-100 disabled:bg-slate-100" /></label>
            <div><div className="flex items-center justify-between"><label className="text-sm font-semibold text-slate-700" htmlFor="article-markdown">Markdown content</label><span className="text-xs text-slate-500">Live preview on the right</span></div><textarea id="article-markdown" disabled={busy} value={draft.markdownContent} onChange={event => update('markdownContent', event.target.value)} className="mt-2 min-h-[32rem] w-full rounded-lg border border-slate-300 bg-slate-950 px-4 py-3 font-mono text-sm leading-6 text-slate-100 outline-none transition focus:border-teal-500 focus:ring-2 focus:ring-teal-100 disabled:bg-slate-800" /></div>
          </div>
        </div>

        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-semibold">Article metadata</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold text-slate-700">Difficulty<select disabled={busy} value={draft.difficulty} onChange={event => update('difficulty', event.target.value)} className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 disabled:bg-slate-100">{['Beginner', 'Intermediate', 'Advanced'].map(value => <option key={value}>{value}</option>)}</select></label><label className="text-sm font-semibold text-slate-700">Estimated reading time<input disabled={busy} type="number" min="1" value={draft.estimatedReadingMinutes} onChange={event => update('estimatedReadingMinutes', Number(event.target.value))} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5 disabled:bg-slate-100" /></label></div><label className="mt-4 block text-sm font-semibold text-slate-700">Tags<input disabled={busy} value={draft.tags.join(', ')} onChange={event => update('tags', event.target.value.split(',').map(tag => tag.trim()).filter(Boolean))} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5 disabled:bg-slate-100" /></label><label className="mt-4 block text-sm font-semibold text-slate-700">Related articles<select disabled={busy} multiple value={draft.relatedArticleKeys} onChange={event => update('relatedArticleKeys', Array.from(event.target.selectedOptions).map(option => option.value))} className="mt-2 min-h-28 w-full rounded-lg border border-slate-300 px-3 py-2.5 disabled:bg-slate-100">{article.availableRelatedArticles.map(item => <option key={item.id} value={item.key}>{item.title}</option>)}</select></label><label className="mt-4 block text-sm font-semibold text-slate-700">Review notes<textarea disabled={isPublished || busy} value={notes} onChange={event => setNotes(event.target.value)} className="mt-2 min-h-24 w-full rounded-lg border border-slate-300 px-3 py-2.5 disabled:bg-slate-100" /></label></div>

        <div ref={actionRef} className="flex flex-wrap gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><button type="button" disabled={busy} onClick={() => save.mutate()} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-50"><Save size={16} />{save.isPending ? 'Saving...' : 'Save changes'}</button><button type="button" disabled={busy} onClick={reset} className="cursor-pointer rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">Undo unsaved changes</button>{article.status === 'PendingReview' && <><button type="button" disabled={busy} onClick={() => approve.mutate()} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-teal-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-800 disabled:cursor-not-allowed disabled:opacity-50"><CheckCircle2 size={16} />{approve.isPending ? 'Approving...' : 'Approve'}</button><button type="button" disabled={busy} onClick={() => reject.mutate()} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-rose-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-rose-800 disabled:cursor-not-allowed disabled:opacity-50"><XCircle size={16} />{reject.isPending ? 'Rejecting...' : 'Reject'}</button></>}{rejected && <button type="button" disabled={busy} onClick={() => approve.mutate()} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-teal-700 px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"><CheckCircle2 size={16} />Approve</button>}{article.status === 'Approved' && <button type="button" disabled={busy} onClick={() => publish.mutate()} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-teal-700 px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"><Send size={16} />{publish.isPending ? 'Publishing...' : 'Publish'}</button>}{isPublished && <button type="button" disabled={busy} onClick={() => publish.mutate()} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-teal-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-teal-800 disabled:cursor-not-allowed disabled:opacity-50"><Send size={16} />{publish.isPending ? 'Republishing...' : 'Republish'}</button>}<button type="button" disabled={busy} onClick={() => navigate('/review')} className="cursor-pointer px-4 py-2.5 text-sm font-semibold text-slate-600 hover:text-slate-950 disabled:cursor-not-allowed disabled:opacity-50">Cancel</button></div>
    ...
    {showActionJump && <button type="button" onClick={() => actionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' })} className="fixed bottom-6 right-6 z-20 inline-flex cursor-pointer items-center gap-2 rounded-full border border-slate-300/80 bg-white/70 px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-lg backdrop-blur transition hover:bg-white/95 hover:text-teal-800"><span>Actions</span><ArrowDown size={16} /></button>}
  </section>
}
