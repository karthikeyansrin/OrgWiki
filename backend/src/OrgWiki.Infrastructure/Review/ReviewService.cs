...
public sealed class ReviewService(OrgWikiDbContext db, ICurrentUser currentUser) : IReviewService
{
    public async Task<ReviewDashboard> GetDashboardAsync(CancellationToken cancellationToken)
    {
        var articles = await OwnedArticles().Include(x => x.Citations).OrderByDescending(x => x.GeneratedAtUtc).ToListAsync(cancellationToken);
    ...
    {
        var article = await OwnedArticles().Include(x => x.Citations).SingleOrDefaultAsync(x => x.Id == articleId, cancellationToken);
    ...
    {
        var article = await OwnedArticles().SingleOrDefaultAsync(x => x.Id == articleId, cancellationToken);
        if (article is null) return null;
        Validate(request, await OwnedArticles().Where(x => x.GenerationId == article.GenerationId && x.Id != article.Id).Select(x => x.Key).ToListAsync(cancellationToken));
        article.Edit(request.Title.Trim(), request.Summary.Trim(), request.MarkdownContent.Trim(), request.Difficulty, request.EstimatedReadingMinutes, JsonSerializer.Serialize(request.Tags.Select(x => x.Trim()).ToList()), JsonSerializer.Serialize(request.RelatedArticleKeys), currentUser.FullName);
    ...
    public async Task<ReviewArticleDetails?> ApproveAsync(Guid articleId, string? notes, CancellationToken cancellationToken)
    {
        ...
        article.Approve(currentUser.FullName, notes?.Trim()); await db.SaveChangesAsync(cancellationToken); return await GetArticleAsync(articleId, cancellationToken);
    }
    ...
    public async Task<ReviewArticleDetails?> RejectAsync(Guid articleId, string? notes, CancellationToken cancellationToken)
    {
        var article = await OwnedArticles().SingleOrDefaultAsync(x => x.Id == articleId, cancellationToken); if (article is null) return null;
        if (article.Status == GeneratedArticleStatus.Published) throw new InvalidOperationException("Published articles cannot be changed in the review workflow.");
        article.Reject(currentUser.FullName, notes?.Trim()); await db.SaveChangesAsync(cancellationToken); return await GetArticleAsync(articleId, cancellationToken);
    }
    ...
    async Task<ReviewArticleDetails> Details(GeneratedArticle article, CancellationToken cancellationToken)
    {
        ...
        var all = await OwnedArticles().Where(x => x.GenerationId == article.GenerationId).ToListAsync(cancellationToken);
    ...
    private IQueryable<GeneratedArticle> OwnedArticles()
        => db.GeneratedArticles.Where(article => db.KnowledgeGenerations.Any(generation => generation.Id == article.GenerationId && db.KnowledgeAnalyses.Any(analysis => analysis.Id == generation.AnalysisId && db.Uploads.Any(upload => upload.Id == analysis.UploadId && upload.UserId == currentUser.Id))));
