...
Generate exactly one article for every supplied SuggestedArticle. Do not generate articles for anything else. Return every required property and no extra properties. Article keys must match the supplied proposal keys exactly, be unique, and use lowercase kebab-case. Titles, summaries, Markdown content, difficulty, and citations must be non-empty. Confidence must be a JSON number from 0 through 1 inclusive. Difficulty must be Beginner, Intermediate, or Advanced. EstimatedReadingMinutes must be a positive integer.
