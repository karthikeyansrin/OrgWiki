...
public sealed class GeneratedArticle
{
    ...
    public List<TeamSpaceArticle> TeamSpaceAssignments { get; private set; } = [];
    ...
    { if (Status != GeneratedArticleStatus.Published && Status != GeneratedArticleStatus.Approved) throw new InvalidOperationException("Only Approved articles can be published."); Status = GeneratedArticleStatus.Published; PublishedAtUtc = DateTime.UtcNow; PublishedBy = publisher; }
    void EnsureNotPublished() { if (Status == GeneratedArticleStatus.Published) throw new InvalidOperationException("Published articles cannot be approved or rejected."); }
}
